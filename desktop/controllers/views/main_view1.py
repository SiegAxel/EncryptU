...
class MainView(ctk.CTkFrame):
    """Vista principal con diseño inspirado en los mockups proporcionados."""
    ...
    def __init__(self, master, controller, username: str):
        ...
        self.profile_modal: Optional[ctk.CTkToplevel] = None

        self._build_layout()
        self.bind("<Configure>", self._on_resize)

        # Se espera un corto tiempo antes de cargar datos para asegurar que la UI esté lista.
    ...
    def _build_hero_section(self):
        ...
        subtitle = ctk.CTkLabel(
            hero_content,
            text="Gestiona tus credenciales con seguridad total.",
    ...
    def _build_search_box(self, parent: ctk.CTkFrame):
        ...
        search_box.grid(row=0, column=1, sticky="ew", padx=(24, 12), pady=(0, 12))
        ...
        self.status_label_save.grid(row=1, column=1, sticky="w", padx=(24, 12))
        ...
        sync_label.grid(row=2, column=1, sticky="w", padx=(24, 12))

    def _build_user_card(self, parent: ctk.CTkFrame):
        card = ctk.CTkFrame(parent, fg_color="#1A1A1A", corner_radius=20)
        card.grid(row=0, column=2, rowspan=2, sticky="ne", padx=(12, 0))
        ...
        avatar = ctk.CTkButton(
            ...
            command=self._open_profile_view,
        )
    ...
    def _render_credential_cards(self):
        ...
        for index, cred in enumerate(self.filtered_credentials):
            ...
            view_button = ctk.CTkButton(
                ...
                command=lambda fid=cred["id"], site=cred["site"]: self.view_copy_action(fid, site),
                font=self.fonts["button"],
            )
            ...
            delete_button = ctk.CTkButton(
                ...
                text_color=COLOR_TEXT_LIGHT,
                command=lambda fid=cred["id"]: self.delete_action(fid),
                font=self.fonts["button"],
            )
            delete_button.pack()
    ...
    def show_status(self, message: str, target: str, color):
        if target != "save" or not self.status_label_save:
            return
        self.status_label_save.configure(text=message)
        if isinstance(color, tuple):
            self.status_label_save.configure(text_color=color[0])
        else:
            self.status_label_save.configure(text_color=color)
    ...
    def _open_profile_view(self):
        if self.profile_modal and self.profile_modal.winfo_exists():
            self.profile_modal.focus_set()
            return

        modal = ctk.CTkToplevel(self)
        self.profile_modal = modal
        modal.title("Perfil de usuario")
        modal.geometry("560x480")
        modal.resizable(False, False)

        container = ctk.CTkFrame(modal, fg_color=COLOR_WHITE, corner_radius=20)
        container.pack(expand=True, fill="both", padx=16, pady=16)

        header = ctk.CTkFrame(container, fg_color="transparent")
        header.pack(fill="x")

        back_button = ctk.CTkButton(header, text="< Volver al inicio", command=modal.destroy, fg_color="transparent", hover_color="#F3F4F6", text_color=COLOR_ACCENT, font=self.fonts["button"])
        back_button.pack(anchor="w", padx=12, pady=(12, 0))

        banner = ctk.CTkFrame(container, fg_color="transparent", height=200)
        banner.pack(fill="x", padx=12, pady=(0, 12))
        banner.pack_propagate(False)

        banner_image = ctk.CTkImage(self.hero_bg_source.resize((520, 200)), size=(520, 200))
        banner_label = ctk.CTkLabel(banner, text="", image=banner_image)
        banner_label.place(relx=0, rely=0, relwidth=1, relheight=1)
        banner_label.image = banner_image

        avatar_wrapper = ctk.CTkFrame(banner, fg_color="#FFFFFF", width=160, height=160, corner_radius=80)
        avatar_wrapper.place(relx=0.5, rely=0.6, anchor="center")
        avatar_wrapper.grid_propagate(False)

        inner_avatar = ctk.CTkFrame(avatar_wrapper, fg_color=COLOR_ACCENT, width=140, height=140, corner_radius=70)
        inner_avatar.place(relx=0.5, rely=0.5, anchor="center")
        inner_avatar.grid_propagate(False)

        initials_label = ctk.CTkLabel(inner_avatar, text=self._get_initials(self.username), font=self.fonts["hero_title"], text_color=COLOR_TEXT_LIGHT)
        initials_label.place(relx=0.5, rely=0.5, anchor="center")

        info = ctk.CTkFrame(container, fg_color="transparent")
        info.pack(fill="both", expand=True, padx=24, pady=(36, 0))
        info.grid_columnconfigure(0, weight=1)

        name_label = ctk.CTkLabel(info, text=self._format_username(self.username), font=self.fonts["section"], text_color=COLOR_TEXT_DARK)
        name_label.grid(row=0, column=0, sticky="n")

        email_label = ctk.CTkLabel(info, text=self.username, font=self.fonts["card_sub"], text_color=COLOR_MUTED)
        email_label.grid(row=1, column=0, sticky="n", pady=(4, 24))

        subscription = ctk.CTkFrame(info, fg_color="#F1F5F9", corner_radius=18)
        subscription.grid(row=2, column=0, sticky="ew", pady=(0, 16))
        subscription.grid_columnconfigure(0, weight=1)

        subscription_label = ctk.CTkLabel(subscription, text="Suscripcion Activa", font=self.fonts["card_title"], text_color=COLOR_TEXT_DARK)
        subscription_label.grid(row=0, column=0, sticky="w", padx=16, pady=(16, 4))

        subscription_meta = ctk.CTkLabel(subscription, text="Plan Premium", font=self.fonts["card_sub"], text_color=COLOR_MUTED)
        subscription_meta.grid(row=1, column=0, sticky="w", padx=16, pady=(0, 12))

        cancel_button = ctk.CTkButton(subscription, text="Cancelar Suscripcion", fg_color="transparent", hover_color="#FDECEC", text_color=COLOR_ACCENT, font=self.fonts["button"], command=lambda: self.show_temp_popup("Un asesor se pondra en contacto contigo."))
        cancel_button.grid(row=2, column=0, sticky="w", padx=16, pady=(0, 16))

        logout_button = ctk.CTkButton(info, text="Cerrar Sesion", fg_color="transparent", hover_color="#E5E7EB", text_color=COLOR_ACCENT, font=self.fonts["button"], command=lambda: (modal.destroy(), self.logout_action()))
        logout_button.grid(row=3, column=0, pady=(24, 0))
    ...
    def _on_resize(self, event):
        if event.widget is self:
            ...
            if width != self._hero_bg_width and self.hero_bg_label and self.hero_bg_source:
                ...
                self.hero_bg_label.image = self.hero_bg_image
